...
1. 将申请的证书、秘钥文件上传到 *C:\data\wampserverpps\cert* 目录
...
6. 将下面的 HTTPS 配置文件模板（`<VirtualHost *:443>...</VirtualHost>`），增加到 httpd-vhosts.conf 文件中(不能删除原有内容)
    ...
    DocumentRoot "C:\data\wampserver\www"
    <Directory "C:\data\wampserver\www">
    Options Indexes FollowSymlinks
    AllowOverride All
    Require all granted
    </Directory>
    SSLEngine on
    SSLCertificateFile  "C:\data\wampserverpps\cert\cert.pem"
    SSLCertificateKeyFile  "C:\data\wampserverpps\cert\key.pem"
    SSLCertificateChainFile  "C:\data\wampserverpps\cert\chain.pem"
    </VirtualHost>
    ```
...
3. 备份原来的 C:\data\wampstack\php 文件夹，再将该文件夹下所有文件删除，将新版 PHP 文件解压到这个文件夹里
...
以将原目录 *C:\data\wampserver* 下的 **www** 迁移到 *d:\www* 目录下为例，具体步骤如下：
