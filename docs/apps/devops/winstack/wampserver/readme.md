...
部署  WampServer 之后，请参考下面的步骤快速入门。

## 准备

1. 在云控制台获取您的 **服务器公网IP地址** 
2. 在云控制台安全组中，确保 **Inbound（入）规则** 下的 **TCP:80** 端口已经开启
3. 在服务器中查看 WampServer 的 默认账号和密码
...
*  虚拟机主机配置文件：*C:\data\wampserver\bin\apache\apache2.4.x\conf\extra\httpd-vhosts.conf* 
...
2. 删除示例网站 *C:\data\wampserver\www\www.example.com* 下的所有文件（保留目录）
...
1. 使用 远程桌面 连接服务器，在 C:\data\wampserver\www 下新建一个网站目录，假设命令为“mysite2”
   ![](https://libs.websoft9.com/Websoft9/DocsPicture/zh/wampserver/wampserver-addmysite2-websoft9.png)

2. 将本地网站源文件上传到：*C:\data\wampserver\www\mysite2* 
   ...
   ![](https://libs.websoft9.com/Websoft9/DocsPicture/zh/wampserver/wampserver-addmorevhostconfig-websoft9.png)
     ...
     * **没有域名，通过 http://IP/mysite2 访问网站**  
     ...
     请将下面 Alias 模板拷贝到 httpd-vhosts.conf 中，并修改其中的 /path, Directory等项的值
      ...
      ```
      Alias /sitename C:\data\wampserver\www\mysite2
      <Directory "C:\data\wampserver\www\mysite2">
	     Options Indexes FollowSymlinks
	     AllowOverride All
	     Require all granted
...
网站存放目录（建议）： *C:\data\wampserver\www*  
示例网站目录： *C:\data\wampserver\www\www.example.com*  

> 通过 *http://公网IP地址* 访问的就是示例网站 

#### Apache{#apache}

**Apache 虚拟主机配置文件**： *C:\data\wampserver\bin\apache\apache2.4.x\conf\extra\httpd-vhosts.conf*  
![](https://libs.websoft9.com/Websoft9/DocsPicture/zh/wampserver/wampserver-vhost-websoft9.png)

Apache 主配置文件： *C:\data\wampserver\bin\apache\apache2.4.x\conf\httpd.conf*   
Apache 日志文件： *C:\data\wampserver\logs*  
Apache 模块目录： *C:\data\wampserver\bin\apache\apache2.4.x\modules*  
...
ServerName www.mydomain.com
#ServerAlias other.mydomain.com
DocumentRoot "C:\data\wampserver\www\www.example.com"
ErrorLog "logs\www.mydomain.com_error_apache.log"
CustomLog "logs\www.mydomain.com_apache.log" common
<Directory "C:\data\wampserver\www\www.example.com">
   Options Indexes FollowSymlinks
   AllowOverride All
   Require all granted
</Directory>
...
PHP 配置文件： *C:\data\wampserver\bin\php\php7.x.x\php.ini*  
PHP 扩展目录： *C:\data\wampserver\bin\php\php7.x.x\ext*   
PHP 扩展配置文件： *C:\data\wampserver\bin\php\php7.x.x\ext\phpForApache.ini*  

PHP 扩展启用或关闭，通过修改 PHP 配置文件实现

#### MySQL

MySQL 安装路径：*C:\data\wampserver\bin\mysql*  
MySQL 数据文件：*C:\data\wampserver\bin\mysql\mysql5.x.x\data*  
MySQL 配置文件：*C:\data\wampserver\bin\mysql\mysql5.x.x\my.ini*    
MySQL 可视化管理地址: *http://服务器公网IP/phpmyadmin*

#### MariaDB

MariaDB 安装路径：*C:\data\wampserver\bin\mariadb*  
MariaDB 数据文件：*C:\data\wampserver\bin\mariadb\mariad10.x.x\data*  
MariaDB 配置文件：*C:\data\wampserver\bin\mariadb\mariad10.x.x\my.ini*    
MariaDB 可视化管理地址: *http://服务器公网IP/phpmyadmin*

#### phpMyAdmin

phpMyAdmin 安装路径: *C:\data\wampserver\apps\phpmyadmin4.x.x*  
phpMyAdmin 配置文件: *C:\data\wampserver\apps\phpmyadmin4.x.x\config.inc.php*   
phpMyAdmin 虚拟主机配置文件: *C:\data\wampserver\alias\phpmyadmin.conf*   
...
在服务器 *C:\data\wampserver* 目录下查看安装目录名称。
