...
Certbot可以提供 [Let's Encrypt](https://letsencrypt.org/) 免费的证书部署，只需一条命令就可以启动证书部署.
...
1. 安装Certbot  
参考：https://certbot.eff.org/ ，直接选择软件和操作系统即可。

2. 连接服务器，运行自动部署证书命令 
   ```
   sudo certbot
   ```
3. 根据提示输入或选择正确的项，直至提示成功
